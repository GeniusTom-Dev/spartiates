...
<div class="h-[40vh] w-full flex flex-col justify-center items-center">
    <form class="flex flex-col items-center space-y-5 w-4/5 sm:w-3/5 md:w-2/5 lg:w-1/3 xl:w-1/4" id="verificationForm" method="post">
        ...
        <label for="code" class="w-full">
            <span class="text-2xl md:text-3xl lg:text-4xl">Entrez le code de la session:</span>
            <input class="rounded-xl w-full mt-2 p-2 text-lg md:text-xl lg:text-2xl border-gray-300" type="text" id="code" name="code" inputmode="numeric" pattern="\d{6}" minlength="6" maxlength="6" oninput="this.value = this.value.replace(/[^0-9]/g, '').slice(0, 6);" required>
        </label>
